...
OFFLINE_THRESHOLD = 300  # 5 min

# ── GEMINI via raw HTTP (no SDK threading issues) ────────────────────
...
def gemini_reply(system_prompt, history, user_text):
    ...
    try:
        ...
        if "candidates" in data:
            ...
            print(f"[Gemini] Bad response: {data}")
            return None
...
def build_system(biz, price_list, greeting):
    ...
    pl       = price_list or "Price list not set — tell the customer the owner will share details."
    greeting = greeting   or DEFAULT_GREETING
    return (
        f"You are the AI sales assistant for {biz}. "
        f"The owner is offline. You handle customer inquiries.\n\n"
        f"FIRST MESSAGE GREETING: {greeting}\n\n"
        f"PRICE LIST (only use these, never invent prices):\n{pl}\n\n"
        f"RULES: Be friendly and concise. "
        f"If something is not in the price list, say the owner will follow up. "
        f"Keep replies under 100 words. Use 1-2 emojis naturally."
    )
    ...
    try:
        ...
        async def on_message(event):
            try:
                ...
                if not cfg:
                    print(f"[!] No config for {seller_id}")
                    return
                ...
                if not auto_reply_on:
                    print(f"[~] Auto-reply OFF for {seller_id}")
                    return
                if is_online(seller_id):
                    print(f"[~] Seller {seller_id} ONLINE — skip")
                    return
                ...
                print(f"[MSG] {customer_name}: {user_text[:60]}")
                ...
                if not reply:
                    reply = (f"Hi! 👋 Thanks for contacting {biz or 'us'}. "
                             f"The owner will get back to you shortly!")
